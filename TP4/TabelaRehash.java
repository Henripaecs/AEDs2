...
class HashRehash {
    ...
    public void inserir(Show show) {
        ...
        if (tabela[pos] == null) {
            tabela[pos] = show;
        } else if (tabela[pos].getTitle().equals(show.getTitle())) {
            return; // já existe
        } else {
            int pos2 = rehash(show.getTitle());
            if (tabela[pos2] == null) {
                tabela[pos2] = show;
            } else if (tabela[pos2].getTitle().equals(show.getTitle())) {
                return; // já existe
            }
            // Se ambas ocupadas e não é duplicata, ignora inserção
        }
    }
    ...
    public int[] pesquisarDetalhado(String title) {
        ...
        int pos2 = rehash(title);
        if (tabela[pos2] != null && tabela[pos2].getTitle().equals(title)) {
...
public class TabelaRehash {
    public static void main(String[] args) throws Exception {
        ...
        // Log
        try (PrintWriter log = new PrintWriter("846431_hashRehash.txt")) { 
            log.printf("846431\t%.3f\n", tempo);
        }
    }
}