...
int main() {
    ...
    while (fgets(entrada, sizeof(entrada), stdin)) {
        ...
        for (int i = 0; i < totalShows; i++) {
            if (strcmp(shows[i].title, entrada) == 0) {
    ...
    // busca por títulos (title)
    while (fgets(entrada, sizeof(entrada), stdin)) {
        entrada[strcspn(entrada, "\n")] = 0;
        if (strcmp(entrada, "FIM") == 0) break;

        int comparacoesBin = 0;
        int pos = pesquisaBinaria(lista, tamLista, entrada, &comparacoesBin);

        printf("%s\n", pos != -1 ? "SIM" : "NAO");
    }

    FILE *log = fopen("846431_bianrio.txt", "w");
    if (log) {
